...
class open_db():
    ...
    def select_Map_01(self, filename, uuid):
        ...
        sqlshell = "SELECT  name,create_time,modify_time,resolution,width,height,left,top,data,original_data," \
                   "pbstream FROM Map a,ProxyMap b ,MapData c " \
                   "WHERE a.map_id=b.map_id AND a.map_id=c.map_id AND  a.old =0 AND b.uuid='{}'".format(uuid)
        cursor.execute(sqlshell)
        result = cursor.fetchall()
        return result

    def select_Map_10(self, filename, uuid):
        self.oldpath_name = []
        conne = sqlite3.connect(filename)
        cursor = conne.cursor()
        sqlshell = "SELECT  name,create_time,modify_time,resolution,width,height,left,top,data,original_data," \
                   "pbstream, slam_data FROM Map a,ProxyMap b ,MapData c " \
                   "WHERE a.map_id=b.map_id AND a.map_id=c.map_id AND  a.old =0 AND b.uuid='{}'".format(uuid)
        cursor.execute(sqlshell)
        result = cursor.fetchall()
        return result
    ...
    def select_Path(self, filename, uuid):
        self.oldpath_name = []
        oldpath = self.select_OldPath_(filename, uuid)
        oldpath1 = self.select_Zone_path(filename, uuid)
        oldpath2 = self.select_record_Path(filename, uuid)
        oldpath3 = self.select_Area_Zone_Path(filename, uuid)
        oldpath = list(set(oldpath1 + oldpath + oldpath2 + oldpath3))
        oldpath = tuple(oldpath)
        # print(tuple(oldpath))
        if len(oldpath) == 1:
            oldpath = (int(oldpath[0]))

            conne = sqlite3.connect(filename)
            cursor = conne.cursor()
            sqlshell = "SELECT path_id,name,type,wall_follow_type, cover_area,length FROM Path a,ProxyMap b " \
                       "WHERE   a.old= 0 AND b.map_id = a.map_id AND b.uuid= '{}' AND a.path_id NOT IN  ({})".format(
                uuid, oldpath)
            cursor.execute(sqlshell)
            result = cursor.fetchall()
            return result
    ...
    def select_area_Path(self, filename, uuid):
        self.oldpath_name = []
        conne = sqlite3.connect(filename)
        cursor = conne.cursor()
        sqlshell = "SELECT length, path_id_data FROM Area a,ProxyMap b " \
    ...
    def select_Zone(self, filename, uuid):
        zonelen = self.select_Area_Zone(filename, uuid)
        if len(zonelen) == 1:
            zonelen = (int(zonelen[0]))

            conne = sqlite3.connect(filename)
            cursor = conne.cursor()
            sqlshell = "SELECT zone_id ,name,path_id,type,config_id FROM Zone a,ProxyMap b " \
                       "WHERE b.map_id=a.map_id AND  a.old =0 AND  b.uuid='{}' " \
                       "AND a.zone_id NOT IN  ({})".format(uuid, zonelen)
            cursor.execute(sqlshell)
            result = cursor.fetchall()
            return result
        conne = sqlite3.connect(filename)
        ...
        sqlshell = "SELECT zone_id ,name,path_id,type,config_id FROM Zone a,ProxyMap b " \
                   "WHERE b.map_id=a.map_id AND  a.old =0 AND  b.uuid='{}' " \
                   "AND a.zone_id NOT IN  {}".format(uuid, tuple(zonelen))
        cursor.execute(sqlshell)
        result = cursor.fetchall()
        return result
    ...
    def select_statistics_record_01(self, filename):
        statistics = "a.record_id,type, b.upload_time,c.uuid,name,path_id,create_time,config_id,area," \
    ...
    def select_Zone_path(self, filename, uuid):
        a = []
        conne = sqlite3.connect(filename)
        cursor = conne.cursor()
    ...
    def select_Area_Zone(self, filename, uuid):
        a = []
        conne = sqlite3.connect(filename)
        cursor = conne.cursor()
        sqlshell = "SELECT zone_id FROM Area a,ProxyMap b " \
                   "WHERE b.map_id=a.map_id AND  a.old =1 AND  b.uuid='{}' ".format(uuid)
        cursor.execute(sqlshell)
        result = cursor.execute(sqlshell)
        for i in result:
    ...
    def select_Area_Zone_Path(self, filename, uuid):
        a = []
        conne = sqlite3.connect(filename)
        cursor = conne.cursor()
        sqlshell = "SELECT path_id FROM  Zone  WHERE Zone.zone_id IN " \
                   "(SELECT zone_id FROM Area a,ProxyMap b WHERE b.map_id=a.map_id AND  a.old =1 AND  b.uuid='{}')" \
                   "".format(uuid)
        cursor.execute(sqlshell)
        result = cursor.execute(sqlshell)
        for i in result:
    ...
    def select_record_Path(self, filename, uuid):
        a = []
        conne = sqlite3.connect(filename)
        cursor = conne.cursor()
        sqlshell = "SELECT path_id FROM Record a,ProxyMap b " \
                   "WHERE b.map_id=a.map_id AND  a.old =1 AND  b.uuid='{}' ".format(uuid)
        cursor.execute(sqlshell)
        result = cursor.execute(sqlshell)
        for i in result:
    ...
    def select_OldPath_(self, filename, uuid):
        sum = []
        suma = []
        result = []
        conne = sqlite3.connect(filename)
    ...
    def select_user(self, filename):
        ...
        conne = sqlite3.connect(filename)
        cursor = conne.cursor()
        sqlshell = "SELECT user_id ,name,password,number,authority,company,status FROM User WHERE old=0"
        cursor.execute(sqlshell)
        result = cursor.execute(sqlshell)
        for i in result:
            a.append(i)
...
if __name__ == '__main__':
    db_nums = 1
    db_banben = input("请输入升级之前的版本（0.1或1.0）：")
    ...
    for root, dir, files in os.walk(loaclfile):
        ...
        if "user.db" in files:
            if "升级前HF" + db_banben in root and db_banben == "0.1":
                old_user_file = os.path.join(root, "user.db")
            if "升级前HF" + db_banben in root and db_banben == "1.0":
                old_user_file = os.path.join(root, "user.db")

    if old_mapnums == new_db_mapnums and db_banben == "0.1":
        for i in old_map_names:
            ...
            elif operator.eq(new_task_len, old_task_len):
                print("对比地图{}中的Task表4个元素，共{}个数据".format(i, len(old_task) * 4) + "Passed")
            else:
        ...
        for i in range(5):
            if operator.eq(old_config[i], new_config[i]):
        ...
        try:
            ...
            old_user=old_db.select_user(old_user_file)
            new_user_file = os.path.join(new_db_file, "user.db")
            new_user=new_db.select_user(new_user_file)
            if old_user == new_user:
                print("对比user.db中的user表元素:Passed")
            else:
                print(old_user)
                print(new_user)
                print("对比statistics.db中的CleanStatistic表元素:Failed，请查看")
            ...
            pass
    elif old_mapnums == new_db_mapnums and db_banben == "1.0":
        for i in old_map_names:
            ...
            if new_area == old_area:
                ...
                print(new_db_files + "Area 表Failed")
            new_map = new_db.select_Map_10(new_db_files)
            old_map = old_db.select_Map_10(old_db_file, i)
            if new_map == old_map:
                print("对比地图{}中的Map表12个元素，共{}个数据".format(i, len(old_map) * 12) + "Passed")
            else:
                print(new_db_files + "Map表Failed")
        ...
        old_config = old_db.select_config_10(old_db_file)
        config_file = os.path.join(new_db_file, "config.db")
        new_config = new_db.select_config_10(config_file)
        for i in range(7):

            if old_config[i] == new_config[i]:
                # print(old_config[i],new_config[i])
                print("对比config.db中的{}表的元素:Passed".format(list(old_db.config10.keys())[i]))
            else:
                print(old_config[i], new_config[i])
                print(config_file + list(old_db.config10.keys())[i] + "Failed")
        old_record = old_db.select_statistics_record_01(old_db_file)
        new_record_file = os.path.join(new_db_file, "statistics.db")
        new_record = new_db.select_statistics_record_01(new_record_file)
        ...
        try:
            ...
            if old_user == new_user:
                ...
                print(old_user)
                print(new_user)
                print("对比user.db中的user表元素:Failed，请查看")
        except NameError:
            pass
    else:
        print(old_db_file)
